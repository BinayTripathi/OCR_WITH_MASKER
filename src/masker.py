...
def maskImage(base64Image, coordinates):
    ...
    topLeftPoint = coordinates[3]
    bottomRightPoint = coordinates[1]
    #cv2.rectangle(mask, (185,2975), (830, 2876), 0, -1)
    cv2.rectangle(mask, (topLeftPoint['x'],topLeftPoint['y']), (bottomRightPoint['x'], bottomRightPoint['y']), 0, -1)

    # compute the bitwise AND using the mask
    masked_img = cv2.bitwise_and(img,img,mask = mask)

    # display the mask, and the output image -----------
    #viewMaskingResults(img, mask, masked_img)
    # ------------------
...
def viewMaskingResults(img, mask, masked_img):
